...
const csv = fs.readFileSync(path.join(root, "tests", "fixtures", "BOTBLOX_ENET.csv"), "utf8");
const sheet = api.normalizeSheetMatrix(api.parseDelimitedText(csv));
if (sheet.importError) throw new Error(sheet.importError);
const result = api.compileSheetHarnessResult(sheet, "BOTBLOX_ENET.csv");
const outputDir = path.join(root, "examples", "generated");
fs.mkdirSync(outputDir, { recursive: true });
const cleanGeneratedText = (value) => `${String(value).split(/\r?\n/).map((line) => line.trimEnd()).join("\n").trim()}\n`;
fs.writeFileSync(path.join(outputDir, "BOTBLOX-ENET.svg"), cleanGeneratedText(api.buildSheetHarnessSvg(result)));
fs.writeFileSync(path.join(outputDir, "BOTBLOX-ENET.drawio"), cleanGeneratedText(api.buildSheetDrawioXml(result)));
console.log(`Generated BOTBLOX ENET assets in ${outputDir}`);
